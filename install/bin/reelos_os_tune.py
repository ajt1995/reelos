...
SYSCTL_DROPIN_DEFAULT = "/etc/sysctl.d/99-reelos-hdd.conf"
...
def sysctl_hdd_text() -> str:
    return (
        "# ReelOS — spinning HDD writeback tuning to mitigate D-state stalls\n"
        "vm.dirty_ratio = 5\n"
    ...
    *,
    ...
    if plan["zram"]:
        ...
        try:
            ...
            tmp_z = zpath.with_suffix(".tmp")
            tmp_z.write_text(zram_conf_text(ram_gb=float(prof.get("ram_gb") or plan["zram_size_mb"] / 1024)), encoding="utf-8", newline="\n")
            os.replace(tmp_z, zpath)
    ...
    if plan["disable_kdump"]:
        ...
        try:
            ...
            tmp_g = gpath.with_suffix(".tmp")
            tmp_g.write_text(grub_nokdump_text(), encoding="utf-8", newline="\n")
            os.replace(tmp_g, gpath)
        ...
        try:
            ...
            tmp_k = kpath.with_suffix(".tmp")
            tmp_k.write_text(kdump_tools_text(), encoding="utf-8", newline="\n")
            os.replace(tmp_k, kpath)
    ...
    if plan.get("sysctl_hdd"):
        spath = _path("REELOS_SYSCTL_DROPIN", SYSCTL_DROPIN_DEFAULT)
        try:
            spath.parent.mkdir(parents=True, exist_ok=True)
            tmp_s = spath.with_suffix(".tmp")
            tmp_s.write_text(sysctl_hdd_text(), encoding="utf-8", newline="\n")
            os.replace(tmp_s, spath)
            out["wrote"].append(str(spath))
            out["sysctl_hdd"] = str(spath)
            if install_packages:
                try:
                    subprocess.run(["sysctl", "-p", str(spath)], check=False, capture_output=True, timeout=15)
                except (OSError, subprocess.TimeoutExpired):
                    pass
        except OSError as e:
            out["sysctl_error"] = str(e)

    if out.get("ok"):
        try:
            stamp.parent.mkdir(parents=True, exist_ok=True)
            tmp_stamp = stamp.with_suffix(".tmp")
            tmp_stamp.write_text(json.dumps(plan_key) + "\n", encoding="utf-8", newline="\n")
