...
def catchup_scan_offset(progress_path: Path | None = None) -> int:
    path = progress_path or PROGRESS_PATH
    try:
        row = json.loads(path.read_text(encoding="utf8"))
        return int(row.get("scanOffset") or 0)
    except (OSError, ValueError, TypeError, json.JSONDecodeError):
        return 0


def rotate_scan_targets(targets: list, cap: int, offset: int = 0) -> tuple[list, int, int]:
    """HDD cap this catch-up; next run starts after the last taken folder."""
    rows = list(targets or [])
    if not cap or len(rows) <= cap:
        return rows, 0, 0
    n = len(rows)
    start = int(offset or 0) % n
    ordered = rows[start:] + rows[:start]
    taken = ordered[: int(cap)]
    return taken, n - len(taken), (start + len(taken)) % n
...
def _expand_scan_folders(
    ...
    *,
    ...
    if cap and len(targets) > cap:
        offset = catchup_scan_offset() if catch_up else 0
        taken, extra, next_offset = rotate_scan_targets(targets, cap, offset)
        skipped += extra
        if log_wire:
            log_wire(f"sonarr import cap {cap} of {len(targets)} folders offset {offset}")
        targets = taken
    if catch_up:
        write_library_progress(
            ...
            scanOffset=next_offset if cap and skipped else 0,
        )
    return targets
...
def _self_test() -> int:
    ...
    class Matching(unittest.TestCase):
        ...
        def test_catchup_cap_rotates_past_first_six(self):
            folders = [f"/symlinks/sonarr/s{n}" for n in range(1, 9)]
            first, extra, nxt = rotate_scan_targets(folders, 6, 0)
            self.assertEqual(first, folders[:6])
            self.assertEqual(extra, 2)
            second, _, nxt2 = rotate_scan_targets(folders, 6, nxt)
            self.assertEqual(second[:2], folders[6:])
            self.assertEqual(rotate_scan_targets(folders[:3], 6, 0), (folders[:3], 0, 0))
