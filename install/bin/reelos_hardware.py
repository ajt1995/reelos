...
STATE = Path(os.environ.get("REELOS_STATE", ".reelos-state" if sys.platform == "win32" else "/var/lib/reelos"))
ROOT = Path(os.environ.get("REELOS_ROOT", "." if sys.platform == "win32" else "/opt/reelos"))
    ...
    if raw is None:
        if sys.platform == "win32":
            try:
                ...
                return int(stat.ullTotalPhys // 1024)
            except Exception:
                pass
        try:
...
def splash_tune_line(profile: dict) -> str:
    ...
    model = str(profile.get("cpu_model") or "")
    product = str(profile.get("product") or "")
    ram_gb = float(profile.get("ram_gb") or 0)
    is_apple = bool(re.search(r"(?:Apple\s+M\d|Apple\s+Silicon|VirtualApple)", f"{model} {product}", re.I))
    if is_apple:
        if ram_gb <= 2.5:
            return "Tuning for Apple Silicon (2GB Minimal VM)…"
        if ram_gb >= 7:
            return f"Tuning for Apple Silicon ({round(ram_gb)}GB Performance VM)…"
        return f"Tuning for Apple Silicon ({max(3, round(ram_gb))}GB VM)…"
...
def limits_for(profile: dict) -> dict:
    ...
    if is_apple:
        if ram_gb <= 2.5:
            ...
            catchup_chunk = min(32, max(16, cpus * 4))
            provision_chunk = min(60, max(30, cpus * 6))
            transcode_cache_target = "disk"
        else:
            tiny = False
            catchup_mem = "2G" if ram_gb < 20 else "4G"
            ...
            transcode_cache_target = "shm"
    else:
        transcode_cache_target = "disk" if ram_gb < 12 else "shm"
    ...
    return {
        ...
        "transcode_cache_target": transcode_cache_target,
        ...
        "zram": hdd and not is_apple,
        "disable_kdump": tiny,
        "search_parallelism": parallel,
        "indexer_parallelism": parallel,
        "splash_tune": splash_tune_line({"tiny": tiny, "disk_kind": kind, "cpu_model": model, "product": product, "ram_gb": ram_gb}),
    }
