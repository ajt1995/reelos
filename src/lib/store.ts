...
export const UPDATE_NOTES = [
  "1.2.50.57: Bounded dump heal — map UIndex/Torrenting dumps to the named series/movie and ask Sonarr/Radarr to import those folders (API), not docker restart. Catch-up skip is per-season so Rookie S02–S08 Importing actually imports after S01 is in. Coming seasons still skip SeasonSearch (Silo S04 / Rookie S09). Unmonitored released seasons still Request. Movie tmdb-<n> cannot POST as tmdb-tv-<n>. FUSE stale is rshared only — not restarting Sonarr or live Jellyfin. Nanosecond skip if dump folders unchanged. Passengers-class movies same path. Skip 49. Do not house-Apply until told. Gold chrome, prebuilt hashed UI. 1.2.51 parked (was Tron chrome; scrapped — do not reuse).",
