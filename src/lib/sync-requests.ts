...
export function requestNeedsLibraryHandoff(
  row: Pick<MediaRequest, "titleId" | "status" | "engine" | "season">,
  opts: { titles?: Pick<Title, "id" | "ids" | "kind" | "jellyfinId" | "onDiskSeasons" | "importingSeasons">[] } = {},
): boolean {
...
export interface UpcomingMonitoredSeason {
  showId: string;
  showTitle: string;
  poster?: string;
  seasonNumber: number;
  status: "coming_soon" | "monitored";
  airDate?: string;
  reason?: string;
}

export function extractUpcomingMonitoredSeasons(
  requests: MediaRequest[],
  catalog: Title[] = [],
): UpcomingMonitoredSeason[] {
  const results: UpcomingMonitoredSeason[] = [];
  const seen = new Set<string>();

  // 1. From requests that are upcoming/announced or unreleased
  for (const r of requests) {
    if (!r.titleId) continue;
    const isTv = isTvRequestRow(r);
    if (!isTv) continue;
    const reason = String(r.reason || "").toLowerCase();
    const isAnnounced = /announced|not released|upcoming|tba/i.test(reason);
    if (isAnnounced || (r.status === "waiting" && !isInFlightRequest(r))) {
      ...
      results.push({
        ...
        status: "coming_soon",
        airDate: titleObj?.year ? String(titleObj.year) : undefined,
        reason: r.reason || "Upcoming season monitored on Sonarr",
      });
    }
  }

  // 2. From catalog titles with unreleasedSeasons or monitored seasons
  for (const t of catalog) {
    ...
    for (const sn of unreleased) {
      ...
      results.push({
        ...
        seasonNumber: sNum,
        status: "monitored",
        airDate: fact?.airDate || (t.year ? String(t.year) : undefined),
...
export function getMediaLifecycleState(
  title: Pick<Title, "id"> & Partial<Pick<Title, "ids" | "kind" | "jellyfinId">>,
  request?: Pick<MediaRequest, "status" | "progress" | "reason" | "via"> | null,
  opts: { titles?: Pick<Title, "id" | "ids" | "kind" | "jellyfinId">[]; inLibrary?: boolean } = {},
): MediaLifecycleState {
  if (title.jellyfinId || opts.inLibrary) return "on_shelf";
  if (opts.titles) {
    for (const t of opts.titles) {
      if (t.jellyfinId && titleMatchesId(t, title.id)) return "on_shelf";
    }
  }
...
export function mergeServerRequests(local: MediaRequest[], server: MediaRequest[]): MediaRequest[] {
  ...
  for (const loc of local) {
    ...
    if (
      loc.status === "downloading" ||
      loc.status === "waiting" ||
      loc.status === "available" ||
      loc.engine === "downloaded"
    ) {
...
export function transferringChipCount(rows: MediaRequest[]): number {
  return collapseHomeRequestCards(
    rows.filter((r) => isInFlightRequest(r) && !isGhostRequestLabel(r.title, r.titleId)),
  ).length;
}
...
export function titleMatchesId(
  t: Pick<Title, "id"> & Partial<Pick<Title, "ids" | "jellyfinId" | "kind">>,
  id: string,
): boolean {
...
export function overlayLibraryPresence(
  ...
  for (const t of opts.titles || []) {
    // Discover/lookup memory is not the JF shelf — must have jellyfinId to count as on the box.
    if (!t.jellyfinId) continue;
