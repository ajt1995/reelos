    ...
    source === "torbox"
      ...
      : source === "alldebrid"
        ...
        : source === "premiumize"
          ? "Premiumize"
          : "Real-Debrid";
  if (via === "cache") return `Ready to stream from ${name}.`;
  const client = source === "torbox" ? "TorBox" : "Decypharr";
  return `Finding release. ${client} will prepare it.`;
}

export function viaLabel(via: RequestVia | undefined, status: string): string | null {
  if (status === "waiting") {
    if (via === "uncached") return "Preparing stream · connecting to sources";
    if (via === "cache") return "Ready in cloud · linking to library";
    return "Waiting for a release";
  }
  if (status === "available") {
    if (via === "cache") return "Ready in cloud · in library";
    if (via === "local") return "Downloaded · in library";
    if (via === "uncached") return "Transferred · in library";
    return "Available now";
  }
  if (status === "downloading") {
    if (via === "cache") return "Ready in cloud · importing";
    if (via === "local") return null;
    if (via === "uncached") return "Preparing stream · transferring";
  }
  return null;
}
