...
const CHANNEL_URL =
  ...
  "https://raw.githubusercontent.com/reelos-org/reelos/main/channel.json";
...
export const checkChannel = createServerFn({ method: "GET" }).handler(async () => {
  ...
  try {
    const urls = [
      "https://raw.githubusercontent.com/reelos-org/reelos/main/channel.json",
      "https://raw.githubusercontent.com/reelos-org/reelos/v1.2.3/channel.json",
      CHANNEL_URL,
      "https://github.com/reelos-org/reelos/raw/main/channel.json",
      "https://cdn.jsdelivr.net/gh/reelos-org/reelos@main/channel.json",
    ];
...
export const pushRequest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!title) return { ok: false as const, error: "Requested title is unavailable" };
    const hash =
      data.hash ||
