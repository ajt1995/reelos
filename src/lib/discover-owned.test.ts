...
test("Discover excludes titles that are currently downloading or in requests", () => {
  const catalog = [
    { id: "tmdb-939243", title: "Sonic the Hedgehog 3", year: 2024, kind: "movie" as const },
    { id: "tmdb-1022789", title: "Inside Out 2", year: 2024, kind: "movie" as const },
  ];
  const downloadingRequest = {
    id: "req-sonic-3",
    titleId: "tmdb-939243",
  ...
  const filtered = filterDiscoverCatalog(catalog, [], [], [downloadingRequest]);
  assert.deepEqual(
