...
export function HouseCard() {
  ...
  const lan = box.ipv4 || "";
  const jf = box.watch || (lan ? `http://${lan}:8080/api/stream` : "");
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <dl className="mt-3 grid gap-1.5 text-sm">
            ...
            <div className="flex justify-between gap-3">
              <dt className="text-muted">Compatible TV apps</dt>
              <dd className="truncate font-mono text-xs">{jf || ":8080/api/stream"}</dd>
            </div>
          </dl>
          <p className="mt-2.5 text-xs text-faint">
            ReelOS handles playback. Compatible apps can connect on this network at {jf || "http://<lan>:8080/api/stream"}; remote access is optional.
