...
export function LibraryPanel() {
  ...
  useEffect(() => {
    void fetch("/api/settings", { cache: "no-store" })
      ...
      .then((j) => {
        setBooksKey(j.googleBooksApiKey || "");
      })
      .catch(() => {});
  }, []);
  return (
    <>
      ...
      {answers.intent.books ? (
        <div className="mt-4">
          <p className="text-sm font-medium">Google Books API key</p>
          <p className="mt-1 text-xs text-muted">
            Optional. Metadata, previews, and buy links — the same as TorBox is a credential, not a novel fetcher. A key
            cannot download Hunger Games onto this box. Seerr and TorBox do not acquire books.
          </p>
          <form
