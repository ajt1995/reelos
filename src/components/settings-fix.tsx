  ...
  return (
    <section className="mt-8">
      ...
      <div className="mt-3 grid gap-2.5">
        <CheckHops />
        <details className="rounded-2xl bg-card px-5 py-4 shadow-[var(--shadow-border)]">
          <summary className="cursor-pointer font-display text-sm font-medium text-muted hover:text-foreground">
            Manual recovery scripts ({REPAIRS.length})
          </summary>
                    ...
                    <li key={r.id} className="flex items-start gap-3 py-2.5 first:pt-0 last:pb-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start gap-3">
                          <p className="min-w-0 flex-1 font-display font-medium text-sm">{r.title}</p>
                          <Button
                            size="sm"
                            disabled={busy !== null}
                        ...
                        {msg[r.id] ? <p className="mt-1 text-xs text-gold">{msg[r.id]}</p> : null}
                      </div>
                    </li>
                  ))}
                </ul>
  ...
  return (
    <div className="rounded-2xl bg-card px-5 py-4 shadow-[var(--shadow-border)]">
      ...
      <p className="mt-1 text-sm text-muted">
        Read-only. Does not guess success. Checks storage, playback, catalog, and configured sources without changing them.
      </p>
      <div className="mt-3">
