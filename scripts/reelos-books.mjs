 ...
 */
import { closeSync, createReadStream, createWriteStream, existsSync, openSync, readdirSync, readFileSync, readSync, statSync } from "node:fs";
...
function mapGutendex(data) {
  ...
  for (const book of (data.results ?? []).slice(0, PER_SOURCE_LIMIT)) {
    ...
    const cover = formats["image/jpeg"] && isHttps(formats["image/jpeg"]) ? formats["image/jpeg"] : null;
    out.push({
...
async function handleDiscover(_req, res) {
  const now = Date.now();
  if (discoverCache && now - discoverCacheTime < DISCOVER_CACHE_TTL_MS) {
    send(res, 200, discoverCache);
    return;
  }
  const unavailable = [];
  const live = [];
  ...
  discoverCache = payload;
  discoverCacheTime = now;
  send(res, 200, payload);
}

function isCorruptBook(abs, name) {
  try {
    ...
    if (name.toLowerCase().endsWith(".epub")) {
      ...
      // EPUB is a zip file, must begin with PK (0x50, 0x4B)
      if (buf[0] !== 0x50 || buf[1] !== 0x4b) return true;
    } else if (name.toLowerCase().endsWith(".pdf")) {
      if (st.size < 100) return true;
      const buf = Buffer.alloc(16);
      const fd = openSync(abs, "r");
      try {
        readSync(fd, buf, 0, 16, 0);
      ...
      if (!buf.toString("utf8", 0, 4).startsWith("%PDF")) return true;
    }
    return false;
  } catch {
    return true;
...
function listLibrary() {
  /** @type {{ title: string, author: string, rel: string, bytes: number, cover?: string | null }[]} */
  const out = [];
  ...
  for (const dir of authors) {
    ...
    for (const name of files) {
      ...
      if (isCorruptBook(fullPath, name)) continue;
      let st;
      try {
      ...
      const title = name.replace(/\.(epub|txt|pdf)$/i, "");
      const author = dir.name;
      const stem = name.replace(/\.[^.]+$/, "");
      let cover = null;
      for (const ext of [".cover.jpg", ".cover.png", ".jpg", ".png"]) {
        const covPath = path.join(folder, stem + ext);
        if (existsSync(covPath)) {
          cover = `/api/books/file?rel=${encodeURIComponent(`${dir.name}/${stem}${ext}`)}`;
          break;
        }
      }
      if (!cover) {
        cover = getStandardEbooksCoverUrl(title, author);
      }
      out.push({
        title,
        author,
...
async function fetchAllowlisted(url) {
  ...
  // Standard Ebooks returns an HTML "Your Download Has Started!" page if requested without ?source=feed
  if (
    (url.hostname === "standardebooks.org" || url.hostname.endsWith(".standardebooks.org")) &&
    url.pathname.endsWith(".epub") &&
    !url.searchParams.has("source")
  ) {
    url.searchParams.set("source", "feed");
  }
  const res = await fetch(url, {
  ...
  if (/text\/html|application\/xhtml\+xml/i.test(ct)) {
    return { error: "The book source returned an HTML page instead of an ebook file." };
  }
...
async function saveToLibrary(book, url, res) {
  ...
  const filePath = path.join(authorDir, stem + ext);
  ...
  const cap = new Transform({
    transform(chunk, _enc, cb) {
      if (firstChunk) {
        firstChunk = false;
        if (ext === ".epub") {
          // Must start with PK (0x50, 0x4B)
          if (chunk.length >= 2 && (chunk[0] !== 0x50 || chunk[1] !== 0x4b)) {
            return cb(new Error("Downloaded content is an HTML page or invalid archive, not an EPUB file."));
          }
        } else if (ext === ".pdf") {
          if (chunk.length >= 4 && !chunk.subarray(0, 4).toString("utf8").startsWith("%PDF")) {
            return cb(new Error("Downloaded content is not a valid PDF file."));
          }
        }
      }
  ...
  const coverUrl = book.cover || getStandardEbooksCoverUrl(book.title, book.author);
  if (coverUrl && isHttps(coverUrl)) {
    fetch(coverUrl, { signal: AbortSignal.timeout(10_000), headers: { "user-agent": BOOK_UA } })
      .then(async (cRes) => {
        if (cRes.ok) {
          const buf = Buffer.from(await cRes.arrayBuffer());
          const coverPath = `${filePath.replace(/\.[^.]+$/, "")}.cover.jpg`;
...
async function handleFile(req, res) {
  ...
  if (rel) {
    ...
    // Auto-repair corrupt stubs (e.g. legacy HTML interstitial pages saved as .epub)
    if (isCorruptBook(abs, name)) {
      const stem = name.replace(/\.[^.]+$/, "");
      const matched = FEATURED_OPEN_BOOKS.find(
        (b) =>
          b.title.toLowerCase() === stem.toLowerCase() ||
          rel.toLowerCase().includes(b.title.toLowerCase()),
      );
      if (matched) {
        try {
          const got = await fetchAllowlisted(new URL(matched.downloadUrl));
          if (!got.error) {
            await saveToLibrary(matched, got.url, got.res);
          }
        } catch (repairErr) {
    ...
    if (isCorruptBook(abs, name)) {
      send(res, 422, { ok: false, error: "This file on the box is corrupted or unreadable." });
      return;
    }
    res.statusCode = 200;
