...
export function parseTitleId(id) {
  ...
  if (/^tmdb-movie-\d+$/.test(s)) {
    const tmdb = s.slice("tmdb-movie-".length);
    return { mediaType: "movie", tmdb, titleId: `tmdb-${tmdb}` };
  }
...
/** DoD 2026-09-08: two 2012–2016 movies + two TV seasons. */
 ...
 * Empty Sonarr stats after a new add (episodeCount 0, no air date) are a
 * released gap — not Coming. Confirm TBA with Seerr/TMDB or a future air /
 * statistics:null placeholder.
 */
export function seasonIsUnreleased(raw, now = Date.now()) {
  ...
  // Sonarr TBA placeholder: statistics is JSON null (Stranger Things S6)
  // or one TBA episode with no air date (Silo S04). episodeCount 0 with no
  // dates is a new add (The Bear S01). A full season with no air dates
  // (Rookie S02 episodeCount 20) is a released gap, not Coming.
  if (raw.statistics === null && files === 0 && !Number.isFinite(prevMs) && !Number.isFinite(nextMs)) {
    return true;
  }
  if (!Number.isFinite(prevMs) && !Number.isFinite(nextMs) && count > 0 && count <= 1) return true;
  return false;
}
...
export function tvRequestReason(row, { series, arrSeriesReady, dumps, torrents, seerrSeason } = {}) {
  ...
  if (seasonUnreleasedForRequest({ sonarrSeason: seasonRow, seerrSeason })) return UNRELEASED_SEASON_COPY;
...
export function overlayPresence(
  ...
  return (rows || []).map((row) => {
    ...
    const seerrSeason = (media?.seasons || []).find((s) => Number(s?.seasonNumber) === Number(row.season));
    if (media) {
    ...
    const reason =
      ...
      tvRequestReason(row, { ...facts, seerrSeason }) ||
    ...
    if (reason && /announced|not released/i.test(reason)) {
      return {
        ...row,
        status: "waiting",
        engine: "queued",
        progress: 0,
        reason: UNRELEASED_SEASON_COPY,
      };
    }
...
export async function attachSeerrDetailTitles(rows, { seerrFetch, key, now = Date.now(), ttlMs = 10 * 60 * 1000, limit = 8 } = {}) {
  ...
  const jobs = [];
  const seen = new Set();
  const inflight = out.filter(
    (row) => row.status === "waiting" || row.status === "downloading" || row.status === "available"
  );
  const ordered = [
