  ...
  while (Date.now() < deadline) {
    try {
      if (typeof fetch === "function") {
        ...
        const r = spawnSync(
          "curl",
          ["-sS", "-o", nullDev, "-w", "%{http_code}", "--max-time", "2", `http://127.0.0.1:${port}/`],
          { encoding: "utf8" },
        );
...
test("hashed gold UI clicks house screenshot: named titles win, Importing ≠ Watch, no 0%", { timeout: 90000 }, async (t) => {
  ...
  const port = String(18900 + Math.floor(Math.random() * 800));
  let chromium;
  try {
    ({ chromium } = await import("playwright"));
  } catch {
  ...
  const child = spawn(process.execPath, ["scripts/with-app-env.mjs", "node", "scripts/reelos-box.mjs"], {
    cwd: root,
  ...
  try {
    ...
    for (const name of ["Silo", "Reacher", "The Rookie"]) {
      ...
      if (name === "The Rookie") {
        ...
        await page.getByText("On disk, importing — Sonarr has not taken the files yet.").waitFor({ timeout: 15000 });
        ...
        await page.getByText("Announced — not released yet. Request cannot grab files that do not exist.").waitFor({ timeout: 15000 });
        ...
        await page.getByRole("button", { name: /Request this season/ }).waitFor({ timeout: 15000 });
