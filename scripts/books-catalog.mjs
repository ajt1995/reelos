...
// Appliance installs keep the durable default. Isolated/local services can
// point the same real Books API at their own state root without writing to a
// machine-global /srv path (notably on Windows development hosts).
...
export function toStandardEbooksSlug(s) {
  return String(s || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
export function getStandardEbooksCoverUrl(title, author) {
...
export function parseStandardEbooksFeed(xml) {
  ...
  for (const match of xml.matchAll(/<entry>([\s\S]*?)<\/entry>/g)) {
    ...
    const allLinks = opdsLinks(entry);
    const acqLinks = allLinks.filter((l) =>
      (l.rel ?? "").startsWith("http://opds-spec.org/acquisition"),
    );
    const epub =
      acqLinks.find((l) => l.type === "application/epub+zip" && /recommended/i.test(l.title ?? "")) ??
      acqLinks.find((l) => l.type === "application/epub+zip");
    if (!epub?.href || !isHttps(epub.href)) continue;

    const imageLink = allLinks.find((l) => (l.rel ?? "").includes("image"));
    const authorBlock = /<author>([\s\S]*?)<\/author>/.exec(entry);
    const author = (authorBlock && tagText(authorBlock[1], "name")) || "Unknown";
    const cover = imageLink?.href && isHttps(imageLink.href) ? imageLink.href : getStandardEbooksCoverUrl(title, author);
...
export function parseGutenbergOpds(xml) {
  ...
  for (const match of xml.matchAll(/<entry>([\s\S]*?)<\/entry>/g)) {
    ...
    const imageLink = links.find((l) => (l.rel ?? "").includes("image"));
    let cover = imageLink?.href ? upgradeHttps(imageLink.href) : null;
    if (!cover) {
      const gidMatch = href.match(/\/ebooks\/(\d+)/);
      if (gidMatch) {
        cover = `https://www.gutenberg.org/cache/epub/${gidMatch[1]}/pg${gidMatch[1]}.cover.medium.jpg`;
      }
    }
    if (cover && !isHttps(cover)) cover = null;
    out.push({
...
function seFeatured(title, author, authorSlug, titleSlug, year) {
  ...
  return {
    ...
    year,
    format: "EPUB",
    downloadUrl: `${base}/downloads/${authorSlug}_${titleSlug}.epub?source=feed`,
