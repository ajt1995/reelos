...
test("shellcheck and bash -n on USB/install scripts", (t) => {
  if (!hasBash) {
...
test("autoinstall: reelos, GitHub main, firstboot once, :80 wizard, no secrets", () => {
  ...
  assert.match(install, /reelos-org\/reelos\/archive\/refs\/heads\/main\.tar\.gz/);
...
test("reelos-make-usb.sh dry-run stages nocloud without writing a device", (t) => {
  if (!hasBash) {
    t.skip("bash is required");
    return;
  }
...
test("reelos-make-usb.sh refuses a missing ISO and a partition path", (t) => {
  if (!hasBash) {
    t.skip("bash is required");
    return;
  }
...
test("install-reelos.sh dry-run layouts a repo tree and never stamps provisioned", (t) => {
  if (!hasBash) {
    t.skip("bash is required");
    return;
  }
...
test("install-reelos.sh refuses ota.lock and does not delete it", (t) => {
  if (!hasBash) {
    t.skip("bash is required");
    return;
  }
