...
test("Phase 2: Recently added carousel, shimmer skeletons, in-flight badges, and scan progress", () => {
  const home = read("src/components/home-view.tsx");
  const card = read("src/components/title-card.tsx");
  const poster = read("src/components/poster.tsx");
  ...
  // Recently added row
  assert.match(home, /Row label="Recently added"/);
  assert.match(home, /recentlyAdded/);
  assert.match(types, /dateAdded\?: string/);
  assert.match(lib, /DateCreated/);
  assert.match(lib, /dateAdded: it\.DateCreated/);

  // Poster shimmer skeleton
  assert.match(styles, /@keyframes shimmer/);
  assert.match(styles, /\.shimmer-skeleton/);
  assert.match(poster, /shimmer-skeleton/);

  // In-flight badges and card truth
  assert.match(card, /inFlightBadge/);
  assert.match(card, /isPlayable/);
  assert.match(card, /requestIsWatchableOnShelf/);

  // Library scan progress indicator
  assert.match(home, /libraryCatchup\.status === "running"/);
  assert.match(home, /animate-ping/);
});
