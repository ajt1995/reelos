...
export const LIBRARY_CACHE_FILE = join(dirname(resolveAnswersPath()), "library-shelf.json");
...
export function libraryItemsUrl({ limit } = {}) {
  const q = new URLSearchParams({
    ...
    Fields: "Path,ProviderIds,ImageTags,DateCreated",
    ImageTypeLimit: "1",
    EnableImages: "true",
...
export function looksLikeSceneRelease(name) {
  const s = String(name || "").trim();
  if (!s) return false;
  return QUALITY_CUT.test(s) || /\b(?:2160p|1080p|720p|4k|uhd|remux|bluray|webrip|web-dl|hdtv|hevc|x264|x265)\b/i.test(s);
}
...
export function repairHashTitles(titles, { listFiles } = {}) {
  ...
  return (titles || []).map((t) => {
    if (!t || !(looksLikeHashTitle(t.title) || looksLikeIndexerDump(t.title) || looksLikeSceneRelease(t.title))) return t;
    const files = list(t) || [];
...
export function isJunkLibraryItem(it) {
  const name = String(it?.Name || it?.title || "").trim();
  const path = String(it?.Path || it?.path || "").trim();
  if (
    /\b\d{5}\.m2ts$/i.test(path) ||
    /\b\d{5}\.m2ts$/i.test(name) ||
    /BDMV[/\\\\]STREAM/i.test(path) ||
    /BDMV[/\\\\]STREAM/i.test(name) ||
    /^\d{5}(?:\.m2ts)?$/i.test(name)
  ) {
    return true;
  }
  const tmdb = it?.ProviderIds?.Tmdb || (it?.ids || []).some((i) => /^tmdb-/.test(String(i)));
  const tvdb = it?.ProviderIds?.Tvdb || (it?.ids || []).some((i) => /^tvdb-/.test(String(i)));
  if (tmdb || tvdb) return false;
  if (/^(?:rarbg(\.com)?|sample|etrg|yts|featurette|trailer|video)$/i.test(name)) return true;
  if (/\b(?:rarbg\.com|sample)\.(?:mp4|mkv|avi)$/i.test(path)) return true;
  return false;
}

export function mapJellyfinItems(items, host, { listFiles } = {}) {
  return (items || []).filter((it) => !isJunkLibraryItem(it)).map((it) => {
...
export function mapJellyfinItem(it, host) {
  ...
  return {
    ...
    path: it.Path || "",
    jellyfinId: it.Id,
    dateAdded: it.DateCreated ? String(it.DateCreated) : undefined,
    maxQuality: "4k",
    popularity: 50,
    genres: [],
  };
...
export function homeShelfRows(titles) {
  ...
  return list.filter((t) => {
    ...
    if (
      /\b\d{5}\.m2ts$/i.test(pathStr) ||
      /\b\d{5}\.m2ts$/i.test(titleStr) ||
      /BDMV[/\\\\]STREAM/i.test(pathStr) ||
      /BDMV[/\\\\]STREAM/i.test(titleStr) ||
      /^\d{5}(?:\.m2ts)?$/i.test(titleStr)
    ) {
      return false;
    }
    if (isDumpTwinCard(t)) {
