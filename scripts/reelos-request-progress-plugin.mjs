...
let _requestListCache = null;
let _requestListCacheAt = 0;

export function invalidateRequestProgressCache() {
  _requestListCache = null;
  _requestListCacheAt = 0;
}

export async function collectRequestList() {
  if (_requestListCache && Date.now() - _requestListCacheAt < 2500) {
    return _requestListCache;
  }
  const key = seerrApiKey();
  if (!key) {
    let debridRequests = [];
    try {
      const res = await fetch("http://127.0.0.1:8282/api/v2/torrents/info", { signal: AbortSignal.timeout(2000) });
      if (res.ok) {
        const torrents = await res.json();
        if (Array.isArray(torrents)) {
          debridRequests = torrents.map((t) => ({
            id: t.hash || t.id,
            titleId: t.name || t.hash,
            title: t.name || "Active Stream",
            status: t.progress >= 1 ? "available" : "downloading",
    ...
    const payload = {
      requests: debridRequests,
      titles: [],
      error: null,
    };
    _requestListCache = payload;
    _requestListCacheAt = Date.now();
    return payload;
  }
  try {
