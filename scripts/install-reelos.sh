...
#
#   curl -fsSL https://raw.githubusercontent.com/reelos-org/reelos/main/scripts/install-reelos.sh | sudo bash
...
GITHUB_TARBALL="${REELOS_GITHUB_TARBALL:-https://github.com/reelos-org/reelos/archive/refs/heads/main.tar.gz}"
...
if need_root; then
  ...
  echo "Or: curl -fsSL https://raw.githubusercontent.com/reelos-org/reelos/main/scripts/install-reelos.sh | sudo bash" >&2
  exit 1
fi
...
find_local_bundle() {
  local cand
  for cand in \
    "$(dirname "$0")/update-bundle.tar.gz" \
    "$(dirname "$0")/reelos-bundle.tar.gz" \
    "/cdrom/nocloud/update-bundle.tar.gz" \
    "/cdrom/nocloud/reelos-bundle.tar.gz" \
    "/opt/reelos/seed/update-bundle.tar.gz" \
    "/nocloud/update-bundle.tar.gz" \
    "/media/nocloud/update-bundle.tar.gz"; do
    if [ -f "$cand" ]; then
      echo "$cand"
      return 0
    fi
...
extract_bundle() {
  ...
  local tmp
  tmp=$(mktemp -d)
  log "extracting local offline installer bundle from $bundle"
  mkdir -p "$tmp/src"
  tar -xzf "$bundle" -C "$tmp/src" --strip-components=1 2>/dev/null || tar -xzf "$bundle" -C "$tmp/src"
  layout_from_repo "$tmp/src"
  local st=$?
  rm -rf "$tmp"
  return $st
...
elif local_bundle=$(find_local_bundle); then
  extract_bundle "$local_bundle"
  log "local offline bundle ($local_bundle)"
else
  fetch_github
  log "GitHub main"
fi
