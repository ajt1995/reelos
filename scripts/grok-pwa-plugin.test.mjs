...
// This app bakes its own share-card identity (src/lib/og/site.json + public/og.jpg),
// and a context without `site`/`cwd` resolves against process.cwd(). A test about the
// injector must not read whatever identity the workspace happens to ship, so it points
// at an empty workspace and states the identity it is exercising itself.
const BARE_WORKSPACE = mkdtempSync(join(tmpdir(), "grok-og-bare-"));
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: BARE_WORKSPACE });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: BARE_WORKSPACE });
  const chunks = [
