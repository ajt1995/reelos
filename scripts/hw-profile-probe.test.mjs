...
test("python probe persists JSON; second ensure is a no-op", () => {
  const self = spawnSync(pythonBin(), [join(root, "daemon/reelos_hardware.py"), "--self-test"], {
    encoding: "utf8",
    cwd: root,
  });
...
test("apple silicon detection and scalable RAM tiers (2GB minimal vs 4GB standard vs 16GB beast)", () => {
  const m1_2gb = hardwareLimits(
    hardwareProfile({ ramKb: 2 * 1024 * 1024, cpus: 2, cpuModel: "Apple M1", diskKind: "ssd" }),
  );
  assert.equal(m1_2gb.isAppleSilicon, true);
  assert.equal(m1_2gb.tiny, true);
  assert.equal(m1_2gb.transcodeCacheTarget, "disk");
  assert.equal(m1_2gb.jellyfinMem, "512M");
  assert.equal(m1_2gb.sonarrMem, "192M");
  assert.equal(m1_2gb.catchupMemoryMax, "512M");
  assert.equal(m1_2gb.zram, false);
  assert.equal(m1_2gb.splashTune, "Tuning for Apple Silicon (2GB Minimal VM)…");

  const m1_4gb = hardwareLimits(
    hardwareProfile({ ramKb: 4 * 1024 * 1024, cpus: 2, cpuModel: "Apple M1", diskKind: "ssd" }),
  );
  assert.equal(m1_4gb.isAppleSilicon, true);
  assert.equal(m1_4gb.tiny, false);
  assert.equal(m1_4gb.transcodeCacheTarget, "disk");
  assert.equal(m1_4gb.jellyfinMem, "1024M");
  assert.equal(m1_4gb.sonarrMem, "256M");
  assert.equal(m1_4gb.splashTune, "Tuning for Apple Silicon (4GB VM)…");

  const m2_16gb = hardwareLimits(
    hardwareProfile({ ramKb: 16 * 1024 * 1024, cpus: 4, cpuModel: "Apple M2 Pro", diskKind: "ssd" }),
  );
  assert.equal(m2_16gb.isAppleSilicon, true);
  assert.equal(m2_16gb.tiny, false);
  assert.equal(m2_16gb.transcodeCacheTarget, "shm");
  assert.equal(m2_16gb.jellyfinMem, "5G");
  assert.equal(m2_16gb.splashTune, "Tuning for Apple Silicon (16GB Performance VM)…");
});
