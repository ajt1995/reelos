...
import {
  ...
  rank,
  safeSegment,
} from "../server/books-catalog.mjs";

/**
 * Trimmed from a live https://standardebooks.org/feeds/opds/all?query=dracula
 * response. The shape that matters: acquisition hrefs carry a `?source=feed`
 * query string, `rel` is the `/open-access` variant, and `rel` follows `href`
 * on some links and precedes it on others.
 */
...
test("an entry with no epub acquisition link is skipped", () => {
  const xhtmlOnly = SE_FEED.replace(
    /type="application\/epub\+zip"/g,
...
test("link attributes are read by name, in any order", () => {
  const links = opdsLinks(
    '<link rel="self" href="https://example.test/a" type="text/html"/>',
  );
  assert.deepEqual(links, [
    { rel: "self", href: "https://example.test/a", type: "text/html" },
  ]);
});

test("entities in attributes and text are decoded once", () => {
  const feed = SE_FEED.replace(
    "<title>Dracula</title>",
...
test("accents and apostrophes survive sanitising", () => {
  ...
  assert.equal(
    safeSegment("Alice's Adventures", "Untitled"),
    "Alice's Adventures",
  );
});
...
test("the extension follows the file, then the content type", () => {
  assert.equal(
    extensionFor(new URL("https://x.test/a.epub?source=feed"), null),
    ".epub",
  );
  assert.equal(
    extensionFor(new URL("https://x.test/a"), "application/epub+zip"),
    ".epub",
  );
