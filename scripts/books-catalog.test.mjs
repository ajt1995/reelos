 ...
 * response. Acquisition hrefs carry `?source=feed`, rel is `/open-access`, and
 * rel follows href on some links and precedes it on others.
 */
...
test("downloads are allowlisted to the three legal catalogs", () => {
  assert.equal(ownsDownload(new URL("https://standardebooks.org/ebooks/x.epub")), true);
  assert.equal(ownsDownload(new URL("https://www.gutenberg.org/ebooks/345.epub.images")), true);
  assert.equal(ownsDownload(new URL("https://ia800707.us.archive.org/download/x/x.epub")), true);
  assert.equal(ownsDownload(new URL("https://annas-archive.org/md5/abc")), false);
  assert.equal(ownsDownload(new URL("https://libgen.is/book/index.php")), false);
  assert.equal(ownsDownload(new URL("http://192.168.1.4/secret")), false);
});
