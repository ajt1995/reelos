...
test("remove TV: Sonarr unmonitor+delete; JF record purged while /media files stay safe", async () => {
  const calls = [];
  const r = await removeLibraryTitle({
    titleId: "tvdb-153021",
    confirm: true,
  ...
  assert.equal(r.steps.jellyfin.deleted, true);
  assert.ok(calls.some((c) => c[0] === "arr" && c[1] === "DELETE" && String(c[2]).includes("deleteFiles=false")));
  assert.equal(calls.some((c) => c[0] === "jf"), true);
...
test("Multi-version cascade deletion purges all Jellyfin IDs (1080p + 4K)", async () => {
  const deletedIds = [];
  const r = await removeLibraryTitle({
    titleId: "tmdb-157336",
    jellyfinId: "jf-4k-id",
    mediaType: "movie",
    confirm: true,
    ids: ["tmdb-157336", "jf-1080p-id"],
    jellyfinDeleteItem: async (id) => {
      deletedIds.push(id);
    },
    jellyfinGetItem: async (id) => ({ Id: id, Path: `/mnt/symlinks/radarr/Interstellar.${id}` }),
  });
  assert.equal(r.ok, true);
  assert.ok(deletedIds.includes("4k-id"), "purges 4K JF ID");
  assert.ok(deletedIds.includes("1080p-id"), "purges 1080p JF ID");
  assert.equal(r.steps.jellyfin.deleted, true);
});

test("cancelRequest purges Seerr request, Seerr media, and Radarr download queue", async () => {
  const seerrCalls = [];
  const arrCalls = [];
  const r = await cancelRequest({
    ...
    seerrKey: "mock-key",
    seerrFetch: async (path, opts) => {
      seerrCalls.push({ path, method: opts?.method || "GET" });
      if (path === "/api/v1/movie/157336") {
        return {
          json: {
            id: 157336,
            mediaInfo: {
    ...
    radarrKey: "mock-radarr-key",
    fetchArr: async (url, key, timeout, opts) => {
      arrCalls.push({ url, method: opts?.method || "GET" });
      if (url.includes("/queue?")) {
        return {
          records: [
            { id: 101, movie: { tmdbId: 157336 }, title: "Interstellar.2014" },
            { id: 102, movie: { tmdbId: 999999 }, title: "Other.Movie" },
          ],
        };
      }
      return { ok: true };
  ...
  // Seerr delete for request 42 and media 99
  assert.ok(seerrCalls.some((c) => c.path === "/api/v1/request/42" && c.method === "DELETE"));
  assert.ok(seerrCalls.some((c) => c.path === "/api/v1/media/99" && c.method === "DELETE"));
  // Radarr queue delete for item 101
  assert.ok(arrCalls.some((c) => c.url.includes("/queue/101") && c.method === "DELETE"));
  // Does NOT delete queue 102 (different movie)
  assert.ok(!arrCalls.some((c) => c.url.includes("/queue/102")));
});

test("cancelRequest handles missing engines gracefully without error", async () => {
  const r = await cancelRequest({
    id: "seerr-10",
    titleId: "tmdb-999",
  });
