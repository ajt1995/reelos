...
export function curatorStateDir(state = process.env.REELOS_STATE) {
  if (state) return String(state).replace(/[\\/]$/, "");
  if (process.platform === "win32") {
    return ".reelos-state";
  }
  return "/var/lib/reelos";
}
...
export function normalizeCurator(raw) {
  ...
  const curationWeights = raw?.curationWeights && typeof raw.curationWeights === "object" ? { ...raw.curationWeights } : {};
  const tasteVibe = typeof raw?.tasteVibe === "string" ? raw.tasteVibe : "balanced";
  const mediaPriorities = raw?.mediaPriorities && typeof raw.mediaPriorities === "object" ? { ...raw.mediaPriorities } : undefined;
...
export function writeCurator(doc, state) {
  ...
  const target = curatorPath(state);
  const lock = `${target}.lock`;
  
  // Spin-lock for file safety
  const maxRetries = 50;
  let retries = 0;
  ...
  try {
    ...
    // On Windows, renameSync can fail if target is locked, try a few times
    let renameRetries = 0;
...
export function updateCuratorTaste({ profileId, tasteVibe, curationWeights, mediaPriorities } = {}, state) {
  const cur = readCurator(state);
  const profiles = { ...(cur.profiles || {}) };
  if (profileId) {
    profiles[profileId] = {
      ...(profiles[profileId] || {}),
      tasteVibe: tasteVibe || profiles[profileId]?.tasteVibe || "balanced",
      curationWeights: {
        ...(profiles[profileId]?.curationWeights || {}),
        ...(curationWeights || {}),
      },
      ...(mediaPriorities ? { mediaPriorities } : {}),
      updatedAt: Date.now(),
  ...
  const next = writeCurator(
    {
      ...cur,
      profiles,
      curationWeights: { ...(cur.curationWeights || {}), ...(curationWeights || {}) },
      tasteVibe: tasteVibe || cur.tasteVibe || "balanced",
      ...(mediaPriorities ? { mediaPriorities } : {}),
    },
    state,
  );
...
export function curatorPublic(doc = emptyCurator()) {
  ...
  return {
    ...
    curationWeights: next.curationWeights,
    tasteVibe: next.tasteVibe,
