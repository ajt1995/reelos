...
import { findLocalIntelligencePack, mediaToolEnvironment } from "./local-tool-discovery.mjs";
...
function main(argv) {
  ...
  if (process.platform === "win32" && !process.env.REELOS_STATE) {
    process.env.REELOS_STATE = join(root, ".reelos-state");
  } else if (process.platform === "darwin" && !process.env.REELOS_STATE) {
    process.env.REELOS_STATE = process.env.HOME ? join(process.env.HOME, "Library", "Application Support", "reelos") : join(root, ".reelos-state");
  }
  for (const [key, value] of Object.entries(mediaToolEnvironment(root))) {
    if (!process.env[key]) process.env[key] = value;
  }
  const intelligencePack = findLocalIntelligencePack(root);
  if (!process.env.REELOS_MODEL_PACK_DIR && intelligencePack) {
    process.env.REELOS_MODEL_PACK_DIR = intelligencePack.directory;
  }
  ...
  const isCmdOrBat = typeof bin === "string" && (bin.toLowerCase().endsWith(".cmd") || bin.toLowerCase().endsWith(".bat"));
  const isVite = typeof bin === "string" && (bin === "vite" || bin.toLowerCase().includes("vite"));
  const shell = process.platform === "win32" && (isVite || isCmdOrBat);
  const child = spawn(bin, args, { stdio: "inherit", env, shell });
