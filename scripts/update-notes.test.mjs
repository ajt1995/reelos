...
test("2.0.0 is newer than 1.2.50.40; rollback is the older stable", () => {
  assert.ok(cmpVer("2.0.0", "1.2.50.40") > 0);
  assert.ok(cmpVer("1.2.50.40", "2.0.0") < 0);
  assert.equal(isRollback("2.0.0", "1.2.50.40", false), true);
  assert.equal(isRollback("2.0.0", "1.2.50.40", true), false);
  assert.equal(isRollback("1.2.50.40", "1.2.50.40", false), false);
  const pending = pendingNotes(
    ["2.0.0: Arena chrome and Books.", "1.2.50.40: Stable."],
    "1.2.50.40",
    "2.0.0",
  );
  assert.equal(pending.length, 1);
