...
test("OTA cleaner twins, never /media or ota.lock, self-test", () => {
  ...
  const py = spawnSync(pythonBin(), [join(root, "daemon/reelos_ota_clean.py"), "--self-test"], {
    encoding: "utf8",
    cwd: root,
  });
  assert.equal(py.status, 0, py.stdout + py.stderr);
  const tune = spawnSync(pythonBin(), [join(root, "daemon/reelos_os_tune.py"), "--self-test"], {
    encoding: "utf8",
    cwd: root,
  });
...
test("cleaner does not kill live Jellyfin/Seerr; drops overseerr/jackett", () => {
  const plan = spawnSync(pythonBin(), [join(root, "daemon/reelos_ota_clean.py"), "--plan"], {
    encoding: "utf8",
    cwd: root,
...
print("ok")
`;
    const r = spawnSync(pythonBin(), ["-c", py], { encoding: "utf8", cwd: root });
...
test("update splash covers Apply; library banner does not freeze Request", () => {
  ...
  assert.match(req, /apiRouter\.post\("\/api\/request"/);
