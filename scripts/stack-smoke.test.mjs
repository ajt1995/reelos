...
test("stack: VERSION / channel / stamps agree (1.2.50.44)", () => {
  ...
  assert.equal(ver, "1.2.50.44");
  assert.equal(chan.version, "1.2.50.44");
  ...
  assert.match(stamp, /SHIPPED_VERSION = "1\.2\.50\.44"/);
  assert.match(stamp, /LATEST_VERSION = "1\.2\.50\.44"/);
  assert.match(store, /SHIPPED_VERSION = "1\.2\.50\.44"/);
  assert.match(store, /LATEST_VERSION = "1\.2\.50\.44"/);
  ...
  assert.match(chan.notes[0], /Splash-lock|skip-only|catching up/);
  assert.match(chan.notes[1], /tmdb-2059|National Treasure|transferring|Expanse/);
  assert.match(chan.notes[2], /ffprobe|stub|FUSE dumps|concurrency/);
  assert.match(chan.notes[3], /hardware|MemoryMax|concurrency/);
  assert.match(chan.notes[4], /own phone clock|Library catching up|product/);
  assert.match(chan.notes[5], /library catch-up|import after hops/);
  assert.match(chan.notes[6], /TorBox/);
  assert.match(chan.notes[6], /DirectPlay/);
  assert.match(chan.notes[6], /prebuilt hashed UI/);
