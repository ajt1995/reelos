...
test("stack: VERSION / channel / stamps agree (1.2.50.8)", () => {
  const ver = read("VERSION").trim();
  const chan = JSON.parse(read("channel.json"));
  const stamp = read("src/lib/version-stamp.ts");
  const store = read("src/lib/store.ts");
  assert.equal(ver, "1.2.50.8");
  assert.equal(chan.version, "1.2.50.8");
  assert.match(stamp, /SHIPPED_VERSION = "1\.2\.50\.8"/);
  assert.match(stamp, /LATEST_VERSION = "1\.2\.50\.8"/);
  assert.match(store, /SHIPPED_VERSION = "1\.2\.50\.8"/);
  assert.match(store, /LATEST_VERSION = "1\.2\.50\.8"/);
  assert.match(read("HAL.md"), /1\.2\.50\.8/);
});
