...
test("stack: VERSION / channel / stamps agree (1.2.50.59)", () => {
  ...
  assert.equal(chan.version, "1.2.50.59");
  ...
  assert.match(stamp, /SHIPPED_VERSION = "1\.2\.50\.59"/);
  assert.match(stamp, /LATEST_VERSION = "1\.2\.50\.59"/);
  assert.match(store, /SHIPPED_VERSION = "1\.2\.50\.59"/);
  assert.match(store, /LATEST_VERSION = "1\.2\.50\.59"/);
  ...
  assert.match(chan.notes[0], /Like|thumbs up|boosts similar|Not interested/i);
  assert.match(chan.notes[0], /Reset curator preferences/);
  assert.match(chan.notes[0], /Home and Library keep owned titles/);
  assert.match(chan.notes[1], /40.40|Coming|Breaking Bad S02|tmdb-tv|3rd Rock|Dark Knight/i);
  assert.match(chan.notes[1], /Breaking Bad S02/);
  assert.match(chan.notes[1], /Dark Knight is not 3rd Rock/);
  assert.match(chan.notes[1], /Bounded dump heal|dump leftovers|not docker restart/i);
  assert.match(chan.notes[2], /Bounded dump heal|Importing actually imports|not docker restart/i);
  assert.match(chan.notes[3], /On this box|UIndex|dump twins|placeholder|year 0/i);
  assert.match(chan.notes[4], /Unreleased TV seasons|Coming|announced|not released|Silo S04|SeasonSearch/i);
  assert.match(chan.notes[5], /Caddy 2\.6|handle_errors|Updating ReelOS|heredoc|:80 dead/i);
  assert.match(chan.notes[6], /Copying house settings|handle_errors|Updating ReelOS|catch-up banner|Movies and Shows|lookupKey|Passengers|endless scroll/i);
  assert.match(chan.notes[7], /lookupKey|title page|tmdb-\*|jf-\*|white-screen/i);
  assert.match(chan.notes[8], /Passengers|On this box|Watch|Honest Apply splash|tarball\/extract bytes|spinning disk|Discover hides owned/i);
  assert.match(chan.notes[8], /Not interested|curator|Continue watching|Jellyfin-synced/i);
  assert.match(chan.notes[9], /OTA|cleaner|Updating ReelOS|splash/i);
  assert.match(chan.notes[10], /Hands-off home|on this box|pick tonight|Watch|National Treasure/i);
  assert.match(chan.notes[11], /request honesty|movie pages POST|hash paste|National Treasure|Request Sxx/i);
  assert.match(chan.notes[12], /hash|jf-|Unknown on this box|infohash|Rick and Morty/i);
  assert.match(chan.notes[13], /tvdb|Watch|Expanse|title page/i);
  assert.match(chan.notes[14], /Splash-lock|skip-only|catching up/);
  assert.match(chan.notes[15], /tmdb-2059|National Treasure|transferring|Expanse/);
  assert.match(chan.notes[16], /ffprobe|stub|FUSE dumps|concurrency/);
  assert.match(chan.notes[17], /hardware|MemoryMax|concurrency/);
  assert.match(chan.notes[18], /own phone clock|Library catching up|product/);
  assert.match(chan.notes[19], /library catch-up|import after hops/);
  assert.match(chan.notes[20], /TorBox/);
  assert.match(chan.notes[20], /DirectPlay/);
  assert.match(chan.notes[20], /prebuilt hashed UI/);
