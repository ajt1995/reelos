...
test("stack: VERSION / channel / stamps agree (1.2.50.55)", () => {
  ...
  assert.match(chan.notes[0], /Collections and actor search|filmography|TMDB collection/i);
  ...
  assert.match(chan.notes[4], /hash|jf-|Unknown on this box|infohash|Rick and Morty/i);
  assert.match(chan.notes[5], /tvdb|Watch|Expanse|title page/i);
  assert.match(chan.notes[6], /Splash-lock|skip-only|catching up/);
  assert.match(chan.notes[7], /tmdb-2059|National Treasure|transferring|Expanse/);
  assert.match(chan.notes[8], /ffprobe|stub|FUSE dumps|concurrency/);
  assert.match(chan.notes[9], /hardware|MemoryMax|concurrency/);
  assert.match(chan.notes[10], /own phone clock|Library catching up|product/);
  assert.match(chan.notes[11], /library catch-up|import after hops/);
