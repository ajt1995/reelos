...
test("stack: VERSION / channel / stamps agree (1.2.50.49)", () => {
  ...
  assert.equal(ver, "1.2.50.49");
  assert.equal(chan.version, "1.2.50.49");
  ...
  assert.match(stamp, /SHIPPED_VERSION = "1\.2\.50\.49"/);
  assert.match(stamp, /LATEST_VERSION = "1\.2\.50\.49"/);
  assert.match(store, /SHIPPED_VERSION = "1\.2\.50\.49"/);
  assert.match(store, /LATEST_VERSION = "1\.2\.50\.49"/);
  assert.match(read("STATUS.md"), /1\.2\.50\.49/);
  ...
  assert.match(chan.notes[0], /Cloud-only OTA|REELOS_CHANNEL_URL|transferring|\/play/i);
  ...
  assert.match(read("scripts/reelos-lookup-plugin.mjs"), /REELOS_CHANNEL_URL/);
  assert.match(read("daemon/reelos-update.sh"), /REELOS_CHANNEL_URL/);
