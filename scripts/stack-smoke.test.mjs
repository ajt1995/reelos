...
test("stack: VERSION / channel / stamps agree (1.2.50.46)", () => {
  ...
  assert.equal(ver, "1.2.50.46");
  assert.equal(chan.version, "1.2.50.46");
  ...
  assert.match(stamp, /SHIPPED_VERSION = "1\.2\.50\.46"/);
  assert.match(stamp, /LATEST_VERSION = "1\.2\.50\.46"/);
  assert.match(store, /SHIPPED_VERSION = "1\.2\.50\.46"/);
  assert.match(store, /LATEST_VERSION = "1\.2\.50\.46"/);
  ...
  assert.match(chan.notes[0], /hash|jf-|Unknown on this box|infohash|Rick and Morty/i);
  assert.match(chan.notes[1], /tvdb|Watch|Expanse|title page/i);
  assert.match(chan.notes[2], /Splash-lock|skip-only|catching up/);
  assert.match(chan.notes[3], /tmdb-2059|National Treasure|transferring|Expanse/);
  assert.match(chan.notes[4], /ffprobe|stub|FUSE dumps|concurrency/);
  assert.match(chan.notes[5], /hardware|MemoryMax|concurrency/);
  assert.match(chan.notes[6], /own phone clock|Library catching up|product/);
  assert.match(chan.notes[7], /library catch-up|import after hops/);
  assert.match(chan.notes[8], /TorBox/);
  assert.match(chan.notes[8], /DirectPlay/);
  assert.match(chan.notes[8], /prebuilt hashed UI/);
