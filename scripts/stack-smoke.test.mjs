...
test("stack: VERSION / channel / stamps agree (2.0.0)", () => {
  ...
  assert.equal(ver, chan.version);
  assert.match(ver, /^(2\.[0-9.]+|1\.5\.)/);
  ...
  assert.match(stamp, /SHIPPED_VERSION = "(2\.[0-9.]+|1\.5(\.[0-9.]+)*)"/);
  assert.match(stamp, /LATEST_VERSION = "(2\.[0-9.]+|1\.5(\.[0-9.]+)*)"/);
  assert.match(store, /SHIPPED_VERSION = "(2\.0\.0|1\.5(\.[0-9.]+)*)"/);
  assert.match(store, /LATEST_VERSION = "(2\.0\.0|1\.5(\.[0-9.]+)*)"/);
  assert.match(store, /betaChannel: false/);
  assert.match(read("STATUS.md"), /1\.5/);
  assert.match(read("STATUS.md"), /1\.2\.50\.59/);
  assert.match(read("STATUS.md"), /1\.2\.50\.58/);
  assert.match(read("STATUS.md"), /1\.2\.50\.57/);
  assert.match(read("STATUS.md"), /1\.2\.50\.56/);
  assert.match(read("STATUS.md"), /1\.2\.50\.55/);
  assert.match(read("STATUS.md"), /1\.2\.50\.54/);
  assert.match(read("STATUS.md"), /1\.2\.50\.53/);
  assert.match(read("STATUS.md"), /1\.2\.50\.52/);
  assert.match(read("STATUS.md"), /1\.2\.50\.51/);
  ...
  assert.match(read("STATUS.md"), /1\.2\.50\.42/);
  assert.match(read("STATUS.md"), /1\.2\.50\.41/);
  ...
  assert.ok(chan.notes.some((n) => /Five-pillar upgrade|Subtitle engine|Tailscale MagicDNS|Smart Watchlist|TV Play|Disaster recovery/i.test(n)));
  assert.ok(chan.notes.some((n) => /ManualImport|Importing|Coming|fingerprint/i.test(n)));
  assert.ok(chan.notes.some((n) => /Like \/ Not interested|Reset curator/i.test(n)));
  assert.ok(chan.notes.some((n) => /Like|thumbs up|boosts similar|Not interested/i.test(n)));
  assert.ok(chan.notes.some((n) => /Reset curator preferences/.test(n)));
  assert.ok(chan.notes.some((n) => /Home and Library keep owned titles/.test(n)));
  assert.ok(chan.notes.some((n) => /40.40|Coming|Breaking Bad S02|tmdb-tv|3rd Rock|Dark Knight/i.test(n)));
  assert.ok(chan.notes.some((n) => /Breaking Bad S02/.test(n)));
  assert.ok(chan.notes.some((n) => /Dark Knight is not 3rd Rock/.test(n)));
  assert.ok(chan.notes.some((n) => /Bounded dump heal|dump leftovers|not docker restart/i.test(n)));
  assert.ok(chan.notes.some((n) => /Bounded dump heal|Importing actually imports|not docker restart/i.test(n)));
  assert.ok(chan.notes.some((n) => /On this box|UIndex|dump twins|placeholder|year 0/i.test(n)));
  assert.ok(chan.notes.some((n) => /Unreleased TV seasons|Coming|announced|not released|Silo S04|SeasonSearch/i.test(n)));
  assert.ok(chan.notes.some((n) => /Caddy 2\.6|handle_errors|Updating ReelOS|heredoc|:80 dead/i.test(n)));
  assert.ok(chan.notes.some((n) => /Copying house settings|handle_errors|Updating ReelOS|catch-up banner|Movies and Shows|lookupKey|Passengers|endless scroll/i.test(n)));
  assert.ok(chan.notes.some((n) => /lookupKey|title page|tmdb-\*|jf-\*|white-screen/i.test(n)));
  assert.ok(chan.notes.some((n) => /Passengers|On this box|Watch|Honest Apply splash|tarball\/extract bytes|spinning disk|Discover hides owned/i.test(n)));
  assert.ok(chan.notes.some((n) => /Not interested|curator|Continue watching|Jellyfin-synced/i.test(n)));
  assert.ok(chan.notes.some((n) => /OTA|cleaner|Updating ReelOS|splash/i.test(n)));
  assert.ok(chan.notes.some((n) => /Hands-off home|on this box|pick tonight|Watch|National Treasure/i.test(n)));
  assert.ok(chan.notes.some((n) => /request honesty|movie pages POST|hash paste|National Treasure|Request Sxx/i.test(n)));
  assert.ok(chan.notes.some((n) => /hash|jf-|Unknown on this box|infohash|Rick and Morty/i.test(n)));
  assert.ok(chan.notes.some((n) => /tvdb|Watch|Expanse|title page/i.test(n)));
  assert.ok(chan.notes.some((n) => /Splash-lock|skip-only|catching up/.test(n)));
  assert.ok(chan.notes.some((n) => /tmdb-2059|National Treasure|transferring|Expanse/.test(n)));
  assert.ok(chan.notes.some((n) => /ffprobe|stub|FUSE dumps|concurrency/.test(n)));
  assert.ok(chan.notes.some((n) => /hardware|MemoryMax|concurrency/.test(n)));
  assert.ok(chan.notes.some((n) => /own phone clock|Library catching up|product/.test(n)));
  assert.ok(chan.notes.some((n) => /library catch-up|import after hops/.test(n)));
  assert.ok(chan.notes.some((n) => /TorBox/.test(n)));
  assert.ok(chan.notes.some((n) => /DirectPlay/.test(n)));
  assert.ok(chan.notes.some((n) => /prebuilt hashed UI/.test(n)));
...
test("stack: wire-engines parts compile and stay twins after #47/#49/#50", () => {
  for (const rel of ["install/bin/wire-engines.parts", "daemon/wire-engines.parts"]) {
    ...
    const r = spawnSync(pythonBin(), ["-c", "import sys; compile(sys.stdin.read(), 'wire-engines.py', 'exec')"], {
      input: code,
      encoding: "utf8",
    });
