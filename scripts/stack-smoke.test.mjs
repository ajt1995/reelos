...
test("stack: VERSION / channel / stamps agree (1.2.50.45)", () => {
  ...
  assert.equal(ver, "1.2.50.45");
  assert.equal(chan.version, "1.2.50.45");
  ...
  assert.match(stamp, /SHIPPED_VERSION = "1\.2\.50\.45"/);
  assert.match(stamp, /LATEST_VERSION = "1\.2\.50\.45"/);
  assert.match(store, /SHIPPED_VERSION = "1\.2\.50\.45"/);
  assert.match(store, /LATEST_VERSION = "1\.2\.50\.45"/);
  assert.match(read("STATUS.md"), /1\.2\.50\.45/);
  assert.match(read("STATUS.md"), /1\.2\.50\.44/);
  ...
  assert.match(chan.notes[0], /tvdb|Watch|Expanse|title page/i);
  assert.match(chan.notes[1], /Splash-lock|skip-only|catching up/);
  assert.match(chan.notes[2], /tmdb-2059|National Treasure|transferring|Expanse/);
  assert.match(chan.notes[3], /ffprobe|stub|FUSE dumps|concurrency/);
  assert.match(chan.notes[4], /hardware|MemoryMax|concurrency/);
  assert.match(chan.notes[5], /own phone clock|Library catching up|product/);
  assert.match(chan.notes[6], /library catch-up|import after hops/);
  assert.match(chan.notes[7], /TorBox/);
  assert.match(chan.notes[7], /DirectPlay/);
  assert.match(chan.notes[7], /prebuilt hashed UI/);
