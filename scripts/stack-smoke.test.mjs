...
test("stack: VERSION / channel / stamps agree (1.2.50.57)", () => {
  ...
  assert.equal(chan.version, "1.2.50.57");
  ...
  assert.match(stamp, /SHIPPED_VERSION = "1\.2\.50\.57"/);
  assert.match(stamp, /LATEST_VERSION = "1\.2\.50\.57"/);
  assert.match(store, /SHIPPED_VERSION = "1\.2\.50\.57"/);
  assert.match(store, /LATEST_VERSION = "1\.2\.50\.57"/);
  ...
  assert.match(chan.notes[4], /Copying house settings|handle_errors|Updating ReelOS|catch-up banner|Movies and Shows|lookupKey|Passengers|endless scroll/i);
  assert.match(chan.notes[5], /lookupKey|title page|tmdb-\*|jf-\*|white-screen/i);
  assert.match(chan.notes[6], /Passengers|On this box|Watch|Honest Apply splash|tarball\/extract bytes|spinning disk|Discover hides owned/i);
  assert.match(chan.notes[6], /Not interested|curator|Continue watching|Jellyfin-synced/i);
  assert.match(chan.notes[7], /OTA|cleaner|Updating ReelOS|splash/i);
  assert.match(chan.notes[8], /Hands-off home|on this box|pick tonight|Watch|National Treasure/i);
  assert.match(chan.notes[9], /request honesty|movie pages POST|hash paste|National Treasure|Request Sxx/i);
  assert.match(chan.notes[10], /hash|jf-|Unknown on this box|infohash|Rick and Morty/i);
  assert.match(chan.notes[11], /tvdb|Watch|Expanse|title page/i);
  assert.match(chan.notes[12], /Splash-lock|skip-only|catching up/);
  assert.match(chan.notes[13], /tmdb-2059|National Treasure|transferring|Expanse/);
  assert.match(chan.notes[14], /ffprobe|stub|FUSE dumps|concurrency/);
  assert.match(chan.notes[15], /hardware|MemoryMax|concurrency/);
  assert.match(chan.notes[16], /own phone clock|Library catching up|product/);
  assert.match(chan.notes[17], /library catch-up|import after hops/);
  assert.match(chan.notes[18], /TorBox/);
  assert.match(chan.notes[18], /DirectPlay/);
  assert.match(chan.notes[18], /prebuilt hashed UI/);
