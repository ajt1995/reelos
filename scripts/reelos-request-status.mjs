...
export async function waitForArrRow({
  ...
  for (let i = 0; i < tries; i++) {
    ...
    if (rows === null && fetchArr === arrJson) return null;
...
let lastWireImportAt = 0;
let wireImportTimer = null;
const WIRE_IMPORT_COOLDOWN_MS = 20_000;

export function spawnWireImport({ force = false } = {}) {
  const now = Date.now();
  if (!force && now - lastWireImportAt < WIRE_IMPORT_COOLDOWN_MS) {
    if (!wireImportTimer) {
      const wait = Math.max(100, WIRE_IMPORT_COOLDOWN_MS - (now - lastWireImportAt));
      wireImportTimer = setTimeout(() => {
        wireImportTimer = null;
        spawnWireImport({ force: true });
      }, wait);
      if (typeof wireImportTimer?.unref === "function") {
        wireImportTimer.unref();
      }
    }
    return false;
  }
  lastWireImportAt = now;
  const script = [
...
export async function kickArrRecover({
  ...
  if (type === "tv" && sonarrKey && tmdb) {
    if (fetchArr === arrJson) {
      const probe = await fetchArr("http://127.0.0.1:8989/api/v3/system/status", sonarrKey, 300);
      if (!probe) return { ok: true, skipped: "sonarr_offline" };
    }
    const n = Number(season);
    ...
    if (fetchArr === arrJson) {
      const probe = await fetchArr("http://127.0.0.1:7878/api/v3/system/status", radarrKey, 300);
      if (!probe) return { ok: true, skipped: "radarr_offline" };
    }
