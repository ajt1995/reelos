...
- Owner approved optional external-provider functionality behind an off-by-default beta control in Advanced Settings on 2026-09-23. Ordinary first run remains personal/public-domain; opting into beta does not imply an adapter is installed, credentials validated, or bytes available. Disable revokes optional access without deleting personal originals. Preserve this scope explicitly rather than marking unimplemented providers complete.
...
- Both branches and existing household data are preserved. Owner approved tools/dependencies needed to deliver the first version on 2026-09-23, including the missing Linux rendering runtime and scoped hardware-test utilities. This does not authorize paid purchases, new service subscriptions, destructive wipes, signing-identity replacement or public publication without exact scope. Downloads do not relax model provenance/privacy gates.
