...
| --- | --- | --- |
| Media controls/appearance | Existing Media3 and LibVLC3 adapters | Prior Android appearance evidence exists on Pixel/TV; rerun affected shared-control paths after extraction. Windows native dialog/keyed surface and muted decoder warm-up restore local paused pictures; both visible pixel regressions pass on the actual desktop. Sandbox captures failed even before replacement, so original-video visibility is now a prerequisite. Sleep/late-track timing regressions are added. C4 still needs remote restart, Linux, authored-caption styling and keyboard/focus checks; do not substitute clock-only evidence. |
