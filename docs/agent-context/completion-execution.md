...
| --- | --- | --- |
| Media controls/appearance | Existing Media3 and LibVLC3 adapters | Prior Android appearance evidence exists on Pixel/TV; rerun affected shared-control paths after extraction. Windows Main GUI exposed hidden dropdown and stale drawable defects, now addressed by native dialog and keyed surface. New multitrack paused-pixel regression still FAILS although clock/IDs restore; simple color-only pixel test passes. C4 remains blocked on this, remote restart and Linux output; do not substitute clock-only evidence. |
