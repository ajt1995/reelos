...
| --- | --- | --- |
| Media controls/appearance | Existing Media3 and LibVLC3 adapters | Android caption appearance verified on Pixel/TV. VLC3 presets use instance creation, not per-media options; Windows fixture tests verify actual pixels, continuation, sleep expiry and fallback reattachment. Shared desktop controls wired; installed Main interaction, remote restart and Linux surface verification remain open. |
