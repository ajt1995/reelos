...
4. **Flock** `/var/lib/reelos/ota.lock`. Second Apply is refused, not queued. Stale lock without a live apply process is stolen.
...
17. **Compose** — If `/var/lib/reelos/provisioned` and compose yml changed: `docker compose up -d` (profiles) + `wire-engines`. Unchanged yml skips up + indexer test.
