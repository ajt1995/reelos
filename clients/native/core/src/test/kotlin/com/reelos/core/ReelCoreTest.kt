...
class ReelCoreTest {
    ...
    private fun assertLegacyMigration(version: Int) = withFileStore { path ->
        ...
        assertEquals(MotionMode.SUBTLE, core.snapshot.activeProfile?.motionMode)
        assertEquals(BrowsingDensity.COMFORTABLE, core.snapshot.activeProfile?.browsingDensity)
        assertTrue(core.snapshot.activeProfile!!.transparencyEnabled)
