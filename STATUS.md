...
***1.2.50.34 is the ship.*** 2026-09-11. Phone Settings → Updates now shows what this install changed, and after Check, what the available update will change. Channel notes stay owner English (no git log dump). 1.2.50.32 (request-play) and 1.2.50.33 (library remove) left free for sibling agents. Does not take Tron (#52 / #70 / #59). 1.2.51 parked.

## Stamp

- **VERSION / channel:** `1.2.50.34`
- **Base:** `main` at 1.2.50.31
- Left **1.2.50.32** / **1.2.50.33** free (request-play audit / library remove)
...
Settings → Box → Updates always lists **This install** from the bundled `UPDATE_NOTES` for the installed VERSION. After Check, **This update** lists the channel notes for versions newer than the box — not the whole history.

### Check does not dump a git log

`GET /api/update/check` returns `currentNotes` (this stamp) and `notes` (pending target only, owner English). Complements / parked-stamp asides stay in channel.json but are stripped on the phone.

## Proof

```
python3 scripts/check-ota.py .
node --test scripts/update-notes.test.mjs scripts/stack-smoke.test.mjs
```

## Owner / house Apply

**Do not Apply from the agent.** House is still **1.2.50.27**. No house Apply until VM boot. Channel will offer 1.2.50.34 after merge. Owner phone **Check → Apply once**. Do not re-enable `reelos-firstboot`. Do not delete `ota.lock`.
