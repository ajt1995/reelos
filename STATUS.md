...
***1.2.50.51 is the ship.*** Honest Apply splash percent. 50 cleaner/splash stays. OTA cleaner + Updating ReelOS splash until browse/request work. Library banner stays non-blocking. Skip 49 (cloud-only #136). 48 hands-off home stays. Prebuilt hashed UI, no Arena. Settings **Beta channel** still fetches **2.0.0** as a separate tarball. 1.2.51 parked. House is **1.2.50.48**. **Do not house-Apply 50 until told.** Never twice.
...
### Honest Apply splash (1.2.50.51)

Splash percent is real work only: tarball Content-Length vs bytes on disk, then extract uncompressed size vs `du`. Stages without bytes show stage N of 7 plus last heartbeat (e.g. Pulling images · 3/6 · 40s ago). Never a fake climbing 99%. Download 0 bytes for 2+ minutes says stalled. Copy is Tuning for 4GB RAM · spinning disk — 4GB is RAM, not disk size. Settings lists RAM, disk type, and disk size as separate rows. Wizard stays 7 steps. No Arena. Skip 49. Never /media, never ota.lock.
