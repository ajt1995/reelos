...
if [ -f "$HERE/systemd/reelos-battery-guardian.service" ]; then
  cp "$HERE/systemd/reelos-battery-guardian.service" /etc/systemd/system/reelos-battery-guardian.service
fi
if [ -f "$HERE/systemd/reelos-battery-guardian.timer" ]; then
  cp "$HERE/systemd/reelos-battery-guardian.timer" /etc/systemd/system/reelos-battery-guardian.timer
fi
if [ -f "$HERE/systemd/reelos-usb-automount.service" ]; then
  cp "$HERE/systemd/reelos-usb-automount.service" /etc/systemd/system/reelos-usb-automount.service
fi
if [ -f "$HERE/systemd/reelos-usb-automount.timer" ]; then
  cp "$HERE/systemd/reelos-usb-automount.timer" /etc/systemd/system/reelos-usb-automount.timer
fi
...
[ -f "$ROOT/VERSION" ] || echo 1.5.19 >"$ROOT/VERSION"
...
systemctl enable reelos-battery-guardian.timer >/dev/null 2>&1 || true
systemctl enable reelos-usb-automount.timer >/dev/null 2>&1 || true
