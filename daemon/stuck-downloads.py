...
SEARCH_INTERVAL_SEC = int(os.environ.get("REELOS_MISSING_SEARCH_SEC", "7200"))
...
def season_is_unreleased(season, now=None):
    ...
    Empty episodeCount after a new add is a released gap (The Bear S01), not Coming.
    TBA is statistics:null (Stranger Things S6) or a future air date.
    """
    if now is None:
        now = time.time()
    if not isinstance(season, dict):
        return False
    stats_raw = season.get("statistics")
    ...
    if future:
        return True
    if stats_raw is None and files == 0 and prev is None and nxt is None:
        return True
    try:
        count = int(stats.get("episodeCount") or stats.get("totalEpisodeCount") or season.get("episodeCount") or 0)
    except (TypeError, ValueError):
        count = 0
    if prev is None and nxt is None and 0 < count <= 1:
        return True
    return False
...
def log(msg: str) -> None:
    ...
    try:
        ...
        with (STATE / "stuck-downloads.log").open("a", encoding="utf-8") as fh:
            fh.write(line + "\n")
    except OSError:
        pass
...
def rotate_dump_heal(items: list, cursor: int = 0, cap: int = DUMP_HEAL_CAP) -> tuple[list, int, bool]:
    """HDD cap this sweep; next sweep starts after the last taken dump."""
    if not items:
        return [], 0, False
    n = len(items)
    start = int(cursor or 0) % n
    ordered = items[start:] + items[:start]
    taken = ordered[: max(1, int(cap))]
    next_cursor = (start + len(taken)) % n
    return taken, next_cursor, n > len(taken)
...
def _is_potato_hardware() -> bool:
    try:
        prof_path = STATE / "hardware-profile.json"
        if prof_path.exists():
            data = json.loads(prof_path.read_text(encoding="utf-8"))
            if data.get("tiny") or data.get("low_perf") or data.get("box_is_small"):
                return True
...
def _quality_choice() -> str:
    ...
    try:
        val = json.loads(p.read_text(encoding="utf-8")).get("quality")
        if val:
            return str(val)
    except (OSError, json.JSONDecodeError, TypeError, AttributeError):
        pass
    if _is_potato_hardware():
        return "1080p"
    return "hybrid"
...
def relink_hybrid_companions(
    ...
    resolved = str(root).replace("\\", "/").rstrip("/")
...
def _radarr_movie_folder(m: dict, movies_root: str | None = None) -> Path | None:
    ...
    for root in roots:
        folder = root / name
        resolved = str(folder).replace("\\", "/").rstrip("/")
        if resolved == "/media" or "/media/" in resolved + "/":
            continue
        if folder.is_dir():
            return folder
...
def recover_missing_series(
    ...
    *,
    ...
    Fingerprint skip must not sit on Importing dumps — rotate the HDD cap.
    """
    if app["name"] != "sonarr":
        return False
    try:
    ...
    pending: list = []
    searched_count = 0
    max_searches = 1 if _is_potato_hardware() else 5
    for s in series:
        ...
        for season in s.get("seasons") or []:
            ...
            if matching:
                pending.append((s, n, title, matching))
                continue
            ...
            if not _should_search_missing(searched, sk, now):
                continue
            if searched_count >= max_searches:
                continue
            ensure_item_grab_path(app, key, s)
            try:
                call(
    ...
    taken, next_cursor, remainder = rotate_dump_heal(pending, int(state.get("dumpHealTvCursor") or 0))
    state["dumpHealTvCursor"] = next_cursor
    state["dumpHealTvRemainder"] = remainder
    if remainder:
        log(f"sonarr dump heal cap {DUMP_HEAL_CAP} — remainder next sweep")
    for s, n, title, matching in taken:
        folders = [str(p) for p in matching[:DUMP_HEAL_CAP]]
        log(
...
def recover_missing_movies(
    ...
    *,
    ...
    for m in movies:
        ...
        if matching:
            pending.append((m, title, matching))
            continue
        has_torrent, torrent_complete = _torrent_for_title(title, torrents, None)
        if has_torrent and not torrent_complete:
            continue
        ...
        try:
            ...
            log(f"radarr MoviesSearch {title} {type(e).__name__} {e}")
    taken, next_cursor, remainder = rotate_dump_heal(pending, int(state.get("dumpHealMovieCursor") or 0))
    state["dumpHealMovieCursor"] = next_cursor
    state["dumpHealMovieRemainder"] = remainder
    if remainder:
        log(f"radarr dump heal cap {DUMP_HEAL_CAP} — remainder next sweep")
    for m, title, matching in taken:
        folders = [str(p) for p in matching[:DUMP_HEAL_CAP]]
        log(
...
def _self_test() -> int:
    ...
    class Guards(unittest.TestCase):
        ...
        def test_unreleased_season_is_not_searched(self):
            ...
            silo_s04 = {
                ...
                "statistics": {
                    ...
                    "episodeCount": 1,
                    "totalEpisodeCount": 1,
                    "nextAiring": "2027-06-01T00:00:00Z",
                },
            }
            new_add = {
                "seasonNumber": 1,
            ...
            self.assertFalse(season_is_unreleased(new_add, now=now))
            self.assertTrue(season_is_unreleased({"seasonNumber": 6, "monitored": True, "statistics": None}, now=now))
            src = Path(__file__).read_text(encoding="utf-8")
        ...
        def test_rotate_dump_heal_gives_remainder_a_turn(self):
            items = [f"s{n}" for n in range(1, 9)]
            first, cursor, rem = rotate_dump_heal(items, 0, cap=6)
            self.assertEqual(first, items[:6])
            self.assertTrue(rem)
            second, _, rem2 = rotate_dump_heal(items, cursor, cap=6)
            self.assertEqual(second[:2], items[6:])
            self.assertTrue(rem2)
            empty, next_c, rem3 = rotate_dump_heal([], 3, cap=6)
            self.assertEqual(empty, [])
            self.assertEqual(next_c, 0)
            self.assertFalse(rem3)
            src = Path(__file__).read_text(encoding="utf-8")
            series_fn = src[src.find("def recover_missing_series") : src.find("def recover_missing_movies")]
            self.assertIn("rotate_dump_heal", series_fn)
            self.assertNotIn("if skip_dump_import:", series_fn)
            movies_fn = src[src.find("def recover_missing_movies") : src.find("def movie_id_of")]
            self.assertIn("rotate_dump_heal", movies_fn)
            dump_block = movies_fn[: movies_fn.find("if _quality_choice()")]
            self.assertNotIn("if skip_dump_import:", dump_block)
        ...
        def test_relink_hybrid_companions_1080_next_to_4k(self):
            ...
            try:
                ...
                self.skipTest("Symlinks require elevated privileges on Windows host")
