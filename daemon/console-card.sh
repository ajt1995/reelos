#!/bin/bash
# ReelOS tty1 card — hostname, network, Wi-Fi join, URL. Not a desktop.
set -eu
...
draw() {
  ...
  echo
  if command -v qrencode >/dev/null 2>&1 && has_net; then
    qrencode -t ansiutf8 "http://reelos.local" 2>/dev/null || true
  fi
  echo
  echo "1  Join Wi-Fi"
  echo "2  Refresh"
