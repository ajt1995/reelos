#!/bin/bash
# Background diagnose + self-heal. User should not need to tap Heal / Hops / Doctor.
...
truncate_large_logs() {
  for f in "$STATE"/*.log; do
    ...
    if [ "${sz:-0}" -gt 5242880 ]; then
      tail -c 2097152 "$f" > "$f.tmp" 2>/dev/null && mv -f "$f.tmp" "$f" 2>/dev/null || true
      log "truncated large log $(basename "$f") from $sz bytes to 2MB"
    fi
  done
  systemctl reset-failed "session-*.scope" >/dev/null 2>&1 || true
}
