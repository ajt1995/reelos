...
def path_is_forbidden(path: str | os.PathLike[str]) -> bool:
    raw = os.fsdecode(path).replace("\\", "/")
    s = os.path.abspath(os.fsdecode(path)).replace("\\", "/")
    base = os.path.basename(raw)
    if raw == "/media" or raw.startswith("/media/") or s == "/media" or s.startswith("/media/") or s.endswith("/media") or "/media/" in s:
        return True
    if base == "ota.lock" or raw.endswith("/ota.lock") or s.endswith("/ota.lock"):
        return True
    if base == "answers.json":
        return True
    parts = raw.split("/")
    if "firstboot" in parts and raw.endswith(".service"):
        return True
    return False
