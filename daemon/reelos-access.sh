...
if [ "$ACCESS" = "tailscale" ]; then
  ...
  if ! tailscale status >/dev/null 2>&1; then
    SLUG=$(python3 -c 'import uuid; print(uuid.uuid4().hex)' 2>/dev/null || tr -dc 'a-z0-9' </dev/urandom | head -c 32)
    URL=$(tailscale up --hostname="reelos-$SLUG" --timeout=8s 2>&1 | grep -o 'https://login.tailscale.com[^ ]*' | head -1 || true)
