...
import { E as SlidersVertical, Et as Clock, Ft as Check, Gt as ArrowLeft, H as Play, It as Cast, L as RefreshCw, Lt as Captions, Mt as ChevronRight, Nt as ChevronLeft, Ot as Clapperboard, T as Smartphone, m as Tv, n as X, nt as LoaderCircle, o as Volume2, yt as ExternalLink } from "../_libs/lucide-react.mjs";
import { E as cn, K as titleMatchesId, U as showToast, X as useReelStore, j as getTitle, r as Route$2, u as Button } from "./router-M-yvs45k.mjs";
import { n as useExperienceStore } from "./experience-state-BipBJc8l.mjs";
import { i as QrCodeSvg, t as Gate } from "./gate-C-z8U9Xa.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/play._id-ClgSruVS.js
...
function PlayerView({ id, season: propSeason, episode: propEpisode, mediaId: propMediaId }) {
	...
	const cycleSubtitlesRef = (0, import_react.useRef)(cycleSubtitles);
	cycleSubtitlesRef.current = cycleSubtitles;
	const audioPresetRef = (0, import_react.useRef)(audioPreset);
	...
	(0, import_react.useEffect)(() => {
		...
		const handleKeyDown = (e) => {
			...
			if (e.code === "Space" || e.key === "k" || e.key === "K") {
				...
				e.preventDefault();
				if (videoRef.current) {
					const next = Math.min(1, Math.round(((videoRef.current.volume || 1) + .05) * 100) / 100);
				...
				e.preventDefault();
				if (videoRef.current) {
					const next = Math.max(0, Math.round(((videoRef.current.volume || 1) - .05) * 100) / 100);
					videoRef.current.volume = next;
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
				className: cn("absolute top-0 inset-x-0 z-40 flex h-14 shrink-0 items-center justify-between border-b border-white/10 bg-black/70 px-3 sm:px-4 pt-[env(safe-area-inset-top)] pl-[max(0.75rem,env(safe-area-inset-left))] pr-[max(0.75rem,env(safe-area-inset-right))] backdrop-blur-md transition-all duration-300", idle && !showSound && !showRatingPill && !showLibraryKeeping ? "opacity-0 pointer-events-none" : "opacity-100"),
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						availableSubtitles.length > 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
							size: "sm",
							variant: "quiet",
							className: cn("min-h-11 min-w-11 gap-1 text-xs border transition-colors px-2 sm:px-3", subSyncOffsetMs !== 0 || showSubSyncPanel ? "border-gold bg-gold/15 text-gold font-medium" : "border-border/60 text-muted hover:text-foreground"),
						...
						availableSubtitles.length > 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
							size: "sm",
							variant: "quiet",
							className: cn("min-h-11 min-w-11 gap-1 text-xs border transition-colors px-2 sm:px-3", selectedSubIndex !== -1 ? "border-gold bg-gold/15 text-gold font-medium" : "border-border/60 text-muted hover:text-foreground"),
