...
import { N as ChevronDown, P as Check, g as Play, h as Plus } from "../_libs/lucide-react.mjs";
import { A as requestProgressLabel, L as titleMatchesId, M as requestTitleIdForPage, N as showHashAdapter, O as requestMediaTypeForPage, P as showRequestQueueControls, R as titlePresenceKeys, _ as useReelStore, j as requestShowsRetry, n as Route, v as applyTitleRequestPoll, w as isLinkedImportingRequest, x as importingSeasonNumbersForTitle } from "./router-D4zanu1c.mjs";
import { a as cn, i as TitleCard, n as Poster, o as formatRuntime, r as Row, t as CuratorVoteBar } from "./title-card-HtBO2ET4.mjs";
import { a as RemoveFromBox, i as Gate, n as Button, o as jellyfinWatchHref } from "./gate-Ch-Nm4tu.mjs";
import { i as useCurator, n as titleIsCuratorHidden, r as titleIsCuratorLiked } from "./use-curator-tR6hZE93.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/title._id-BQnyEpim.js
...
function useEngineRequest(id, season) {
	...
	const [importingSeasons, setImportingSeasons] = (0, import_react.useState)([]);
	const [unreleasedSeasons, setUnreleasedSeasons] = (0, import_react.useState)([]);
	...
	(0, import_react.useEffect)(() => {
		...
		fetch("/api/library", {
			...
			if (hit) {
				...
				setExtraIds(aliases);
				const importing = seasonNumbersFrom(hit.importingSeasons);
				const disk = seasonNumbersFrom(hit.onDiskSeasons).filter((n) => !importing.includes(n));
				if (importing.length) setImportingSeasons((prev) => [.../* @__PURE__ */ new Set([...prev, ...importing])].sort((a, b) => a - b));
				setOnDiskSeasons((prev) => [.../* @__PURE__ */ new Set([...prev, ...disk])].filter((n) => !importing.includes(n)).sort((a, b) => a - b));
			}
		...
		const poll = () => {
			fetch(`/api/request?${q}`, {
				...
				if (importing.length) setImportingSeasons((prev) => [.../* @__PURE__ */ new Set([...prev, ...importing])].sort((a, b) => a - b));
				if (disk.length || importing.length) setOnDiskSeasons((prev) => [.../* @__PURE__ */ new Set([...prev, ...disk])].filter((n) => !importing.includes(n)).sort((a, b) => a - b));
				const coming = seasonNumbersFrom(j.unreleasedSeasons);
				if (coming.length) setUnreleasedSeasons((prev) => [.../* @__PURE__ */ new Set([...prev, ...coming])].sort((a, b) => a - b));
				...
				const diskNow = disk;
				const pollStatus = season != null && importing.includes(Number(season)) && (j.status === "downloaded" || j.status === "available") ? "downloading" : season != null && (j.status === "downloaded" || j.status === "available") && !diskNow.includes(Number(season)) ? "unknown" : j.status;
...
var UNRELEASED_SEASON_COPY = "Announced — not released yet";
var UNRELEASED_SEASON_CHIP = "Coming";
var IMPORTING_SEASON_CHIP = "Importing";
var IMPORTING_SEASON_COPY = "On disk, importing";
function seasonChipKind(opts = {}) {
...
function seasonShowsRequestButton(opts = {}) {
...
function SeasonEpisodeAccordion({ seasonNumbers, selectedSeason, onSelectSeason, diskSeasons, importingSeasons, unreleasedSeasons, titleId, seasonsLoading, seasonErr, onRetrySeasons, blocked, removedHere, onRequestSeason, onRequestEpisode }) {
	const [open, setOpen] = (0, import_react.useState)(false);
	const [episodes, setEpisodes] = (0, import_react.useState)([]);
	const [unreleasedOpen, setUnreleasedOpen] = (0, import_react.useState)(false);
	...
	const thisImporting = Boolean(importingSeasons?.includes(selectedSeason)) && !thisOnDisk && !thisUnreleased;
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "title-season-chips no-scrollbar",
			children: seasonNumbers.map((n) => {
				...
				const chip = seasonChipLabel({
					onDisk,
					importing: Boolean(importingSeasons?.includes(n)) && !onDisk && !unreleased,
					unreleased,
					removedHere
				});
				return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
					...
					onClick: () => tapSeason(n),
					className: cn("inline-flex h-11 min-h-11 shrink-0 items-center gap-1 rounded-full px-3 text-sm", selected ? "bg-gold text-gold-fg" : "bg-card text-muted shadow-[var(--shadow-border)]"),
					children: [
						"Season ",
						n,
						` · ${chip}`,
			...
			children: [
				...
				!loading && thisUnreleased ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "py-3 text-sm text-muted",
					children: [UNRELEASED_SEASON_COPY, ". Request cannot grab files that do not exist."]
				}) : null,
				!loading && thisImporting ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "py-3 text-sm text-muted",
					children: [IMPORTING_SEASON_COPY, " — Sonarr has not taken the files yet."]
				}) : null,
				!loading && !episodes.length && !err && !thisUnreleased && !thisImporting ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "py-3 text-sm text-muted",
					children: "Episode names land once Sonarr or Seerr has this season. Request this season without hunting."
				}) : null,
				thisUnreleased ? null : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("ul", {
...
function TitleView({ id }) {
	...
	const { inJellyfin, engineStatus, seasonList, onDiskSeasons, importingSeasons, unreleasedSeasons, extraIds: engineIds } = useEngineRequest(id, season);
	const extraIds = [.../* @__PURE__ */ new Set([...titlePresenceKeys(id, resolved?.ids || []), ...engineIds])];
	...
	const requestImporting = (0, import_react.useMemo)(() => importingSeasonNumbersForTitle(id, storeRequests, extraIds), [
		id,
		storeRequests,
		extraIds
	]);
	...
	if (!resolved) {
		if (!seasonsLoading) return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "px-6 py-16",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-muted",
					children: seasonErr || "Seerr did not find that title"
				}),
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Link, {
					to: "/",
					className: "mt-4 ml-4 inline-block text-gold",
					children: "Home"
				})
			]
		});
	...
	const diskSeasons = uniqSeasons([...onDiskSeasons || [], ...resolved.onDiskSeasons || []]).filter((n) => !linkingSeasons.includes(n) && !comingSeasons.includes(n));
	...
	const thisSeasonImporting = Boolean(series && linkingSeasons.includes(season) && !thisSeasonOnBox && !thisSeasonUnreleased);
	const onBox = series ? thisSeasonOnBox : Boolean(resolved.jellyfinId) || inJellyfin || inLibrary;
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "title-page pb-28 md:pb-16",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "title-hero",
				"aria-hidden": "true",
				children: [resolved.poster ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
					src: resolved.poster,
					alt: "",
				...
				}) : null, /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "title-hero-fade" })]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Poster, {
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "mt-5 max-w-xl line-clamp-4 text-[15px] leading-relaxed text-muted md:line-clamp-none",
							children: resolved.overview
						}),
						series && !onBox && !blocked && !request && !thisSeasonUnreleased || !series && !available && !blocked && !request ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "mt-4 text-sm text-gold",
							children: cacheCopy(resolved, source)
						}) : null,
						thisSeasonUnreleased ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "mt-4 text-sm text-muted",
							children: UNRELEASED_SEASON_COPY
						}) : null,
						thisSeasonImporting ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
							className: "mt-4 text-sm text-muted",
							children: IMPORTING_SEASON_COPY
						}) : null,
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								}) : thisSeasonUnreleased ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "inline-flex h-12 items-center rounded-2xl bg-card px-4 text-sm text-muted",
									children: UNRELEASED_SEASON_CHIP
								}) : thisSeasonImporting ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "inline-flex h-12 items-center rounded-2xl bg-card px-4 text-sm text-gold",
									children: IMPORTING_SEASON_CHIP
								...
								resolved ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CuratorVoteBar, {
									placement: "inline",
						...
						!available && !blocked && !thisSeasonUnreleased && hashPaste ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("form", {
			...
			similar.length ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Row, {
					...
					children: similar.map((t) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TitleCard, {
						title: t,
						onVote: voteTitle,
